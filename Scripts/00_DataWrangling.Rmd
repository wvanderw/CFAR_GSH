...
GSH_df$Log10DepthMax <- log10(GSH_df$DepthMax_m_text)
GSH_df$DepthMin_m_text <- as.numeric(GSH_df$DepthMin_m_text)
GSH_df$Log10DepthMin <- log10(GSH_df$DepthMin_m_text)
GSH_df$Log10DepthMax[which(GSH_df$Log10DepthMax == -Inf)] = -1
GSH_df$Log10DepthMin[which(GSH_df$Log10DepthMin == -Inf)] = -1
...
GSH_df <- GSH_df %>% 
  ...
  mutate(Log10MaxSize = log10(MaxLinearDimension),
         ...
         Log10DepthMedian = mean(c(Log10DepthMax, Log10DepthMin)))
#center the Log10 Mean GSH column
GSH_df$Log10MeanGSH_centered <- center_scale(GSH_df$Log10MeanGSH)
#force numeric
GSH_df$Log10MeanGSH_centered <- as.numeric(GSH_df$Log10MeanGSH_centered)
#center scale depths
GSH_df$Log10DepthMax_centered <- (GSH_df$Log10DepthMax - mean(GSH_df$Log10DepthMax, na.rm = TRUE)) / sd(GSH_df$Log10DepthMax, na.rm = TRUE)
GSH_df$Log10DepthMin_centered <- (GSH_df$Log10DepthMin - mean(GSH_df$Log10DepthMin, na.rm = TRUE)) / sd(GSH_df$Log10DepthMin, na.rm = TRUE)
GSH_df$Log10DepthMedian_centered <- (GSH_df$Log10DepthMedian - mean(GSH_df$Log10DepthMedian, na.rm = TRUE)) / sd(GSH_df$Log10DepthMedian, na.rm = TRUE)
...
GSH_df_pgls$Log10MaxSize_centered <- center_scale(GSH_df_pgls$Log10MaxSize)
#force numeric
GSH_df_pgls$Log10MaxSize_centered <- as.numeric(GSH_df_pgls$Log10MaxSize_centered)

#species area
GSH_df_pgls$area_sqkm <- as.numeric(GSH_df_pgls$area_sqkm)
GSH_df_pgls$Log10Area <- log10(GSH_df_pgls$area_sqkm)
GSH_df_pgls$Log10Area_centered <- (GSH_df_pgls$Log10Area - mean(GSH_df_pgls$Log10Area, na.rm = TRUE)) / sd(GSH_df_pgls$Log10Area, na.rm = TRUE)

#center the Log10 CFAR column
GSH_df_pgls$Log10CFAR_centered <- center_scale(GSH_df_pgls$Log10CFAR)
#force numeric
GSH_df_pgls$Log10CFAR_centered <- as.numeric(GSH_df_pgls$Log10CFAR_centered)
...
GSH_df_pgls <- GSH_df_pgls %>% drop_na(Log10DepthMedian_centered, area_sqkm)
