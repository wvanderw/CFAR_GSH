...
```{r}
plot(Log10CFAR ~ Log10MeanGSH_centered, data = GSH_df_pgls)
abline(a = coef(pglsMod1)[1], b = coef(pglsMod1)[2])
```
...
K <- 2
y <- GSH_df_pgls$Log10CFAR
x <- matrix(c(GSH_df_pgls$Log10MeanGSH, GSH_df_pgls$Log10MaxSize_centered), ncol = 2, byrow = F)
