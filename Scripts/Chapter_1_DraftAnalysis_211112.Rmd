...
```{r}
##Install packages
#install.packages(c('ape','phytools','nlme','tidyr','gieger','xlsx','here',
...
#add contrast structure for stan input
...
A <- ape::vcv.phylo(SampleTree)
#correlation matrix (BRMS)
B <- ape::vcv.phylo(SampleTree, corr = TRUE)



```
Link here for brms / stan correlation<https://discourse.mc-stan.org/t/covariance-matrix-phylogenetic-models/20477/2>

## Plot the Tree
```{r}
Tree <- plot.phylo(SampleTree, type = 'fan', show.tip=F, edge.color = rainbow(1200))

png(here("./Data/Output/TreePlot.png"), width = 20, height = 15,
    units = "cm", res = 800, bg = "transparent")

plot.phylo(SampleTree, type = 'fan', show.tip=F, edge.color = rainbow(1200))

dev.off()

#ggsave(here('./Data/Output/TreePlot.png'), plot = Tree, height = 8, width = 8, units = 'in', bg = 'transparent')
...
data_matrix_6 <- model.matrix(~ Log10MeanGSH_centered + PrimaryHabitat, data = GSH_df_pgls)
...
```{r}
## Stan Model 6 -- Log10CFAR ~ Log10MeanGSH_centered + PrimaryHabitat
data_matrix_7 <- model.matrix(~ Log10MeanGSH_centered * PrimaryHabitat, data = GSH_df_pgls)
...
fit7 <- stan(file = './StanModel2.stan', data = stan_data7, warmup = 500, iter = 1000, chains = 4, cores = 4, thin = 1)
