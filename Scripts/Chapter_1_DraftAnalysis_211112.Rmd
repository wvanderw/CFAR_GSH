...
#prior
stan2_prior <- Size_prior

## Stan Model 2 -- Log10CFAR ~ Log10MeanGSH_centered * Log10MaxSize_centered
StanModel2 <- stan_model(file = here('Scripts', 'StanModel2.stan'))

fit2 <- stan(file = './StanModel2.stan', data = stan_data2, warmup = 500, iter = 1000, chains = 4, cores = 2, thin = 1)
...
for (i in 1:500) {
 abline(posterior2$alpha[i], posterior2[i,2], col = "gray", lty = 1)
}

abline(mean(posterior2$alpha), mean(posterior2[,2]), col = 'blue', lw = 2)
...
K <- 2
y <- GSH_df_pgls$Log10CFAR
x <- matrix(c(GSH_df_pgls$Log10MeanGSH_centered, GSH_df_pgls$Log10MaxSize_centered), ncol = 2, byrow = F)
...
abline(mean(posterior3$alpha), mean(posterior3$beta), col = 'green', lw = 2)
...
```{r}
y <- GSH_df_pgls$Log10CFAR
x <- matrix(c(GSH_df_pgls$Log10MeanGSH_centered), ncol = 1)
N <- length(GSH_df_pgls$Binomial)
K <- ncol(x)
...
StanModel4 <- stan_model(file = here('Scripts', 'StanModel3.stan'))
...
for (i in 1:200) {
 abline(posterior4$alpha[i], posterior4$beta[i], col = adjustcolor("gray", alpha = 0.05), lty = 1)
}
...
abline(a = coef(pglsMod2)[1], b = coef(pglsMod2)[2], col = "black", lty = 2, lw = 5)
```
