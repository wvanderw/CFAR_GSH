...
#coastal
  for (i in 1:500) {
 plot_11 <- plot_11 + geom_abline(intercept = fixef(brms_9, summary = F)[i,1], slope = fixef(brms_9, summary = F)[i,2], color = adjustcolor("#1b9e77", alpha = 0.05), lty = 1) 
} 
...
#coastal
plot_11 <- plot_11 + geom_abline(intercept = fixef(brms_9)[1,1], slope = fixef(brms_9)[2,1], color = '#1b9e77', size = 1.5)
#deepwater
...
plot_11b <- plot_11 + geom_point(alpha = 0.5) +
...
I_P
#slope - CFAR ~ GSH
out_SG <- estimate_density(brms_9_post, extend = T, select = 'b_Log10CFAR_centered')
