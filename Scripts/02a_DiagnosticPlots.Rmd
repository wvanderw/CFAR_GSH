...
Prez_plot <- GSH_df_pgls %>% 
  ...
  add_epred_draws(SizeHabitatModel, ndraws = 10) %>% 
  ggplot(aes(x = Log10MeanGSH_centered, y = Log10CFAR, color = PrimaryHabitat), fill = PrimaryHabitat) +
...
```{r}
PPC_plot2 <- ppc_dens_overlay(y = GSH_df_pgls$Log10CFAR, yrep = yrep[1:50, ], alpha = 0.25) +
