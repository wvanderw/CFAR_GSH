...
linear_mod <- lm(LogMeanGSH_centered ~ LogCFAR_centered + LogMaxSize_centered + LogDepthMedian_centered, data = GSH_df_seq)
...
```{r}
brms_Check <- brm(LogMeanGSH_centered ~ LogCFAR_centered + (1|gr(Binomial, cov = D)),
                    data = GSH_df_seq,
                    data2 = list(D = D),
                    family = gaussian(),
                    prior = brms_5_prior,
