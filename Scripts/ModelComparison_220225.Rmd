...
abline(mean(posterior3$alpha), mean(posterior3$beta), col = 6, lw = 2)
...
for (i in 1:500) {
 abline(fixef(Model_size_3, summary = F)[i,1], fixef(Model_size_3, summary = F)[i,2], col = adjustcolor("gray", alpha = 0.05), lty = 1)
}
...
abline(mean(posterior4$alpha), mean(posterior4$beta), col = 6, lw = 2)
...
for (i in 1:200) {
 abline(posterior5$alpha[i], posterior5$beta[i], col = adjustcolor("gray", alpha = 0.15), lty = 1)
} 

abline(mean(posterior5$alpha), mean(posterior5$beta), col = 6, lw = 2)
