...
```{r echo=FALSE}
kbl(fixef(Model_plain, summary = T), booktabs = T, linesep = "", caption = 'brms model summary') %>% 
  kable_styling(full_width = T, latex_options = c('striped','hold_position','scale_down'))
...
```{r echo=FALSE}
kbl(fixef(SizeHabitatModel, summary = T),caption = 'brms model 11 summary', booktabs = T, linesep = "")%>% 
  kable_styling(latex_options = c("scale_down","hold_postion","striped"), full_width = T)
```
